    ...
    fetch(CALENDAR_API_URL)
        ...
        .then(data => {
            const challenges = (Array.isArray(data.challenges) ? data.challenges : [])
                .filter(c => c.special_reward && c.special_reward.amount > 0)
            ...
            const fmtDate = (unix) => unix ? new Date(unix * 1000).toLocaleDateString(undefined, { month: 'short', day: 'numeric' }) : '?';
    ...
    fetch(CALENDAR_API_URL)
        ...
        .then(data => {
            const events = (Array.isArray(data.events) ? data.events : [])
                .filter(e => e.start_time > 0 && e.end_time > 0)
                .sort((a, b) => (a.end_time || 0) - (b.end_time || 0));
            ...
            panel.innerHTML = events.map(e => `
                <div class="reset-row">
                    ...
                    ${e.special_reward && e.special_reward.id === 201 && e.special_reward.amount > 0
                        ? `<div class="reset-row-reward">+${e.special_reward.amount}</div>`
                        : ''}
                </div>
            `).join('');
        })
...
function renderBanners(banners) {
    ...
    panel.innerHTML = active.map(b => {
        ...
        const unitHtml = units.map(u => {
            ...
            const icon = u.icon
                ? `<img src="${u.icon}" alt="" loading="lazy">`
                : `<div class="ac-icon-placeholder">?</div>`;
            return `
